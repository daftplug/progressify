...
<main class="grid xl:grid-cols-7 w-full bg-gray-50 rounded-xl h-[calc(100svh-100px)] overflow-hidden" data-page-id="activation">
  <div class="hidden xl:flex xl:col-span-2 h-[inherit] bg-gray-100 flex-col justify-between gap-y-6 p-6 dark:bg-neutral-950">
    ...
    <div>
      ...
      <img src="<?php echo plugins_url('admin/assets/media/illustrations/plugin-features.svg', $this->pluginFile); ?>" alt="<?php esc_html_e($this->menuTitle, $this->slug); ?>" />
    </div>
    ...
    <div class="max-w-lg flex flex-col justify-center space-y-5">
      ...
      <form id="licenseActivationForm" name="licenseActivationForm" spellcheck="false" autocomplete="off" class="space-y-5">
        <fieldset>
              ...
              <button type="button" class="hs-tooltip-toggle cursor-help ms-1 flex" tabindex="-1">
                ...
                <span class="hs-tooltip-content hs-tooltip-shown:opacity-100 hs-tooltip-shown:visible opacity-0 transition-opacity inline-block absolute invisible max-w-md z-[999999999999] p-3 bg-gray-800 text-xs font-medium text-left text-white rounded shadow-sm dark:bg-neutral-700" role="tooltip">
                  <span class="flex flex-col gap-y-2">
                    <?php _e('To obtain your license key, go to your CodeCanyon downloads page, click "Download" on the plugin, select "License certificate & purchase code," and download the document.', $this->slug); ?>
                  ...
                  <span class="mt-4 flex flex-col gap-y-2">
                    <?php _e('After downloading the document, open it and copy the "Item Purchase Code". This is your license key which you you need to enter in the field below to activate the plugin.', $this->slug); ?>
          ...
          <input id="licenseKey" name="licenseKey" type="text" class="shadow-sm py-2.5 px-3 block w-full border-gray-200 rounded-lg text-sm placeholder:text-gray-400 focus:border-blue-500 focus:ring-blue-500 data-[disabled=true]:opacity-50 data-[disabled=true]:pointer-events-none dark:bg-transparent dark:border-neutral-700 dark:text-neutral-300 dark:placeholder:text-white/60 dark:focus:ring-neutral-600" placeholder="<?php _e('Enter Your License Key', $this->slug); ?>" autocomplete="off" required>
        </fieldset>
        <button type="submit" class="group py-2.5 px-3 w-full inline-flex rounded-lg justify-center items-center gap-x-2 text-sm font-semibold border border-transparent bg-blue-600 text-white hover:bg-blue-700 data-[activating=true]:opacity-50 data-[activating=true]:pointer-events-none focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
          <span class="hidden group-data-[activating=true]:inline-block animate-spin size-4 border-[3px] border-current border-t-transparent text-white rounded-full transition" role="status" aria-label="loading">
            <span class="sr-only"><?php _e('Activating...', $this->slug); ?></span>
          </span>
          <?php _e('Activate License', $this->slug); ?>
        </button>
      </form>
      <p class="text-sm text-gray-500 dark:text-neutral-500">
        <?php _e('Having trouble with activating license?', $this->slug); ?>
