...
<main class="grid xl:grid-cols-7 w-full bg-gray-50 rounded-xl h-[calc(100svh-100px)] overflow-hidden" data-page-id="activation">
  <div class="hidden xl:flex xl:col-span-2 h-[inherit] bg-gray-100 flex-col justify-between gap-y-6 p-6 dark:bg-neutral-950">
    <div class="flex justify-between items-center">
      ...
      <select name="language" data-dp-select='{
        "placeholder": "<?php esc_html_e('Select Language', $this->textDomain); ?>",
        ...
        <option value="English" data-dp-select-option='{"icon": "<img class=\"shrink-0 size-4 rounded-full\" src=\"<?php echo plugins_url('admin/assets/media/icons/flags/1x1/gb.svg', $this->pluginFile); ?>\" alt=\"English\"/>"}' selected>
          <?php esc_html_e('English', $this->textDomain); ?>
        </option>
      </select>
    </div>
    <div>
      ...
      <img src="<?php echo plugins_url('admin/assets/media/illustrations/plugin-features.svg', $this->pluginFile); ?>" alt="<?php esc_html_e($this->menuTitle, $this->textDomain); ?>" />
    </div>
    <div class="flex justify-between gap-x-8">
      <p class="text-xs text-gray-500 dark:text-neutral-500">
        © <?php echo date('Y'); ?> DaftPlug
      </p>
      <button type="button" class="dark:hidden flex hs-dark-mode items-center gap-x-1.5 text-sm text-gray-500 hover:text-gray-800 focus:outline-none focus:text-gray-800" data-hs-theme-click-value="dark">
        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"></path>
        </svg>
        <span class="sr-only"><?php esc_html_e('Dark mode', $this->textDomain); ?></span>
      </button>
      <button type="button" class="hidden dark:flex hs-dark-mode items-center gap-x-1.5 text-sm text-gray-500 hover:text-gray-800 focus:outline-none focus:text-gray-800" data-hs-theme-click-value="light">
        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <span class="sr-only"><?php esc_html_e('Light mode', $this->textDomain); ?></span>
      </button>
    </div>
  </div>
    ...
    <div class="max-w-lg flex flex-col justify-center space-y-5">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-neutral-500">
          <?php esc_html_e('Enter your item purchase code from Envato as your license key to activate the plugin. This will enable in-plugin support and automatic updates.', $this->textDomain); ?>
        </p>
      </div>
      <form id="licenseActivationForm" name="licenseActivationForm" spellcheck="false" autocomplete="off" class="space-y-5">
          ...
          <label class="inline-flex items-center mb-2 text-sm font-medium text-gray-800 dark:text-neutral-200">
            ...
            <div class="hs-tooltip inline-block [--placement:top]">
              <button type="button" class="hs-tooltip-toggle cursor-help ms-1 flex" tabindex="-1">
                ...
                <span class="hs-tooltip-content hs-tooltip-shown:opacity-100 hs-tooltip-shown:visible opacity-0 transition-opacity inline-block absolute invisible max-w-md z-[999999999999] p-3 bg-gray-800 text-xs font-medium text-left text-white rounded shadow-sm dark:bg-neutral-700" role="tooltip">
                  <span class="flex flex-col gap-y-2">
                    <?php _e('To obtain your license key, go to your CodeCanyon downloads page, click "Download" on the plugin, select "License certificate & purchase code," and download the document.', $this->textDomain); ?>
                    <img class="rounded-lg" src="<?php echo plugins_url('admin/assets/media/images/download-license-key.png', $this->pluginFile); ?>" />
                  </span>
                  <span class="mt-4 flex flex-col gap-y-2">
                    <?php _e('After downloading the document, open it and copy the "Item Purchase Code". This is your license key which you you need to enter in the field below to activate the plugin.', $this->textDomain); ?>
                    <img class="rounded-lg" src="<?php echo plugins_url('admin/assets/media/images/license-key.png', $this->pluginFile); ?>" />
                  </span>
                </span>
              </button>
            </div>
          </label>
          <input name="licenseKey" type="text" class="shadow-sm py-2.5 px-3 block w-full border-gray-200 rounded-lg text-sm placeholder:text-gray-400 focus:border-blue-500 focus:ring-blue-500 data-[disabled=true]:opacity-50 data-[disabled=true]:pointer-events-none dark:bg-transparent dark:border-neutral-700 dark:text-neutral-300 dark:placeholder:text-white/60 dark:focus:ring-neutral-600" placeholder="<?php _e('Enter Your License Key', $this->textDomain); ?>" autocomplete="off" required>
        </fieldset>
        <button type="submit" class="group py-2.5 px-3 w-full inline-flex rounded-lg justify-center items-center gap-x-2 text-sm font-semibold border border-transparent bg-blue-600 text-white hover:bg-blue-700 data-[activating=true]:opacity-50 data-[activating=true]:pointer-events-none focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
          <span class="hidden group-data-[activating=true]:inline-block animate-spin size-4 border-[3px] border-current border-t-transparent text-white rounded-full transition" role="status" aria-label="loading">
            <span class="sr-only"><?php _e('Activating...', $this->textDomain); ?></span>
          </span>
          <?php _e('Activate License', $this->textDomain); ?>
        </button>
      </form>
      <p class="text-sm text-gray-500 dark:text-neutral-500">
        <?php _e('Having trouble with activating license?', $this->textDomain); ?>
        <a class="inline-flex items-center gap-x-1 text-sm text-blue-600 decoration-2 hover:underline font-medium focus:outline-none focus:underline dark:text-blue-500" href="mailto:support@daftplug.com" target="_blank">
          <?php _e('Contact Us', $this->textDomain); ?>
          <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
