      ...
      <div class="flex flex-col w-full action-items-container [&:not(.expanded)_.action-item:nth-child(n+5)]:hidden ${actionItems.length > 4 ? '[&:not(.expanded)]:after:absolute [&:not(.expanded)]:after:w-full [&:not(.expanded)]:after:h-[7.5rem] [&:not(.expanded)]:after:bottom-0 [&:not(.expanded)]:after:bg-gradient-to-t [&:not(.expanded)]:after:from-white [&:not(.expanded)]:after:to-transparent [&:not(.expanded)]:after:z-20' : ''}">
        ${actionItems
          .map((actionItem) => {
            ...
            const isGenerateMobileApps = actionItem.id === 'mobileApps';
            ...
            <div class="action-item p-1.5 flex items-center gap-x-2 text-sm font-medium text-gray-800 rounded-lg hover:bg-gray-100 focus:bg-gray-100 focus:outline-none dark:bg-neutral-800 dark:text-neutral-200 dark:hover:bg-neutral-700 dark:hover:text-blue-500 dark:focus:bg-neutral-700 ${isHover ? 'hs-tooltip hs-tooltip-toggle [--placement:top] cursor-help' : 'cursor-pointer'}" ${isClick ? `data-action-type="click" data-navigate-to-page="${actionItem.action.navigateToPage}" data-highlight-element="${actionItem.action.highLightElement}"` : ''}>
              ...
              <div class="grow">
                <span class="${isGenerateMobileApps ? 'bg-clip-text bg-gradient-to-r from-gray-800 to-lime-700 text-transparent font-semibold' : ''}">${actionItem.title}</span>
              </div>
