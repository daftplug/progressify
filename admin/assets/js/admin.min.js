(() => {
  ...
  // admin/assets/js/dev/components/autoHeightTextarea.js
  function initTextareaAutoHeight() {
    handleTextareaAutoHeight();
  }
  function handleTextareaAutoHeight() {
  ...
  // admin/assets/js/dev/components/themeMode.js
  ...
  function handleThemeMode() {
    daftplugAdmin8.find("[data-dp-theme-mode]").each(function() {
      const self2 = jQuery(this);
      const mode = self2.attr("data-dp-theme-mode");
      updateActiveState();
      self2.on("click", (event) => {
        event.stopPropagation();
        if (mode === "auto") {
          localStorage.removeItem("dp_theme");
          localStorage.setItem("dp_theme_mode", "auto");
          applyTheme();
        } else if (mode === "light") {
          localStorage.dp_theme = "light";
          localStorage.removeItem("dp_theme_mode");
          applyTheme();
        } else if (mode === "dark") {
  ...
  function updateActiveState() {
    daftplugAdmin8.find("[data-dp-theme-mode]").attr("data-active", "false");
    if (localStorage.getItem("dp_theme_mode") === "auto") {
      daftplugAdmin8.find('[data-dp-theme-mode="auto"]').attr("data-active", "true");
    } else if (localStorage.dp_theme === "light" || !localStorage.dp_theme) {
      daftplugAdmin8.find('[data-dp-theme-mode="light"]').attr("data-active", "true");
    } else if (localStorage.dp_theme === "dark") {
      daftplugAdmin8.find('[data-dp-theme-mode="dark"]').attr("data-active", "true");
    }
  }

  // admin/assets/js/dev/components/languageSelect.js
  var daftplugAdmin9 = jQuery("#daftplugAdmin");
  var slug = daftplugAdmin9.attr("data-slug");
  function initLanguageSelect() {
    daftplugAdmin9.find("#languageSelect").on("change", changeLanguage);
  }
  async function changeLanguage() {
    const response = await fetch(wpApiSettings.root + slug + "/changeLanguage", {
      ...
      body: JSON.stringify({
        language: daftplugAdmin9.find("#languageSelect").val()
      })
    });
    if (!response.ok) {
  ...
  function showToast(title, description, type, position = "bottom-right", autodismiss = true, dismissible = false, actionMarkup = "") {
    ...
    let toastWrapper = daftplugAdmin10.find(`.toast-wrapper${positionSelector.replace(/\s/g, ".")}`);
    if (!toastWrapper.length) {
      toastWrapper = jQuery(`<div class="toast-wrapper fixed ${positionClasses.join(" ")} space-y-3 z-[99999]"></div>`);
      daftplugAdmin10.append(toastWrapper);
    }
  ...
  function initProcessLicense() {
    daftplugAdmin11.find('form[name="licenseActivationForm"]').on("submit", activateLicense);
  }
  async function sendLicenseProcessRequest(licenseKey, action) {
    ...
    try {
      const response = await fetch(wpApiSettings.root + slug2 + "/requestLicenseProcessing", {
        method: "POST",
        headers: {
          "X-WP-Nonce": wpApiSettings.nonce,
          "Content-Type": "application/json"
  ...
  function navigateToPage(pageId, subPageId = "") {
    const allPages = daftplugAdmin12.find("[data-page-id]");
    const allMenuItems = daftplugAdmin12.find("nav a[data-page-id]");
    const allSubpages = daftplugAdmin12.find("[data-subpage-id]");
    const allSubmenuItems = daftplugAdmin12.find("nav a[data-subpage-id]");
  ...
  function highlightElement(selector) {
    ...
    const spotlight = () => {
      return new Promise((resolve) => {
        ...
        daftplugAdmin12.get(0).appendChild(overlay);
  ...
  function handleNavigation() {
    if (daftplugAdmin13.find('main[data-page-id="activation"]').length) {
  ...
  var daftplugAdmin14 = jQuery("#daftplugAdmin");
  var slug3 = daftplugAdmin14.attr("data-slug");
  ...
  var daftplugAdmin15 = utils_default("#daftplugAdmin");
  var slug4 = daftplugAdmin15.attr("data-slug");
  function initSettings() {
    checkAndGeneratePwaAssetsIfNeeded();
    daftplugAdmin15.find('form[name="settingsForm"]').on("submit", saveSettings);
  }
  async function checkAndGeneratePwaAssetsIfNeeded() {
    ...
    if (data.needsToGenerate) {
      try {
        const iconUrl = daftplugAdmin15.find("#settingAppIcon").find("[data-attachment-holder]").attr("src");
        const backgroundColor = daftplugAdmin15.find('input[name="webAppManifest[appearance][backgroundColor]"]').val();
  ...
  async function saveSettings(e) {
    ...
    const intractableComponents = daftplugAdmin15.find("header, aside, main, footer");
  ...
  var daftplugAdmin16 = jQuery("#daftplugAdmin");
  function initSearch() {
    const searchBox = daftplugAdmin16.find("[data-dp-searchbox]");
  ...
  function buildSettingsItems() {
    const sections = daftplugAdmin16.find("main section");
  ...
  var slug5 = daftplugAdmin17.getAttribute("data-slug");
  var { __ } = wp.i18n;
  var PwaUsersDataManager = class {
    constructor() {
      ...
      this.chart = null;
      this.activeUsersElement = daftplugAdmin17.querySelector("#activePwaUsers");
      this.browserStatsMessage = daftplugAdmin17.querySelector("#browserStatsMessage");
      this.browserStatsContainer = daftplugAdmin17.querySelector("#browserStatsContainer");
      this.chartContainer = daftplugAdmin17.querySelector("#pwaInstallsChart");
      this.periodInputs = daftplugAdmin17.querySelectorAll('input[name="installationPeriod"]');
    }
    init() {
      initApexcharts();
    ...
    generateBrowserMessage(browsers) {
      if (!browsers?.length || !browsers[0].browser_name) {
        return __(`Your PWA hasn't been installed yet, and there are no current users. Make sure Installation Prompts are enabled to encourage users to install your web app.`, slug5);
      }
      const validBrowsers = browsers.filter((b) => b.browser_name);
      const topBrowser = validBrowsers[0];
      if (validBrowsers.length >= 2 && topBrowser.percentage == 50) {
        return __(`Your PWA users are evenly distributed across different browsers. While we strive for broad compatibility, Chrome is recommended as the most PWA-friendly.`, slug5);
      }
      const prefix = topBrowser.percentage == 100 ? __("All", slug5) : __("Most", slug5);
      if (topBrowser.browser_name == "Chrome") {
        return __(`${prefix} of your PWA users (${topBrowser.percentage}%) are using Chrome browser, which is good as Google Chrome is the most PWA-friendly browser.`, slug5);
      } else if (topBrowser.browser_name == "Unknown") {
        return __(`${prefix} of your PWA users (${topBrowser.percentage}%) browsers are unidentifiable, suggesting they are not using a popular browser, which could affect their PWA experience.`, slug5);
      } else {
        return __(`${prefix} of your PWA users (${topBrowser.percentage}%) are using ${topBrowser.browser_name} browser. While we strive for broad compatibility, Chrome is recommended as the most PWA-friendly.`, slug5);
      }
    }
  ...
  var daftplugAdmin18 = document.querySelector("#daftplugAdmin");
  var slug6 = daftplugAdmin18.getAttribute("data-slug");
  var { __: __2 } = wp.i18n;
  var PwaScoreDataManager = class {
    constructor() {
      this.scoreData = null;
      this.pwaScoreResult = daftplugAdmin18.querySelector("#pwaScoreResult");
      this.pwaScoreProgressbar = daftplugAdmin18.querySelector("#pwaScoreProgressbar");
      this.pwaScoreActions = daftplugAdmin18.querySelector("#pwaScoreActions");
    }
    init() {
      this.setupEventListeners();
      this.loadData();
    ...
    generateScoreResult(scoreResult) {
      ...
      <span class="py-1 ps-1.5 pe-2 inline-flex items-center gap-x-1.5 text-xs font-medium rounded-full ${resultColorClass}">
        ...
        <span>${__2(scoreResult, slug6)}</span>
      </span>
    `;
    }
    generateActionItems(actionItems) {
        ...
        <div class="relative px-5 py-6 mt-5 flex flex-col rounded-xl justify-center items-center text-center bg-gray-100">
          <div class="max-w-md mx-auto text-wrap">
            <p class="text-base font-medium text-gray-800 dark:text-neutral-200">
              ${__2(`No Action Items`, slug6)}
            </p>
            <p class="mt-0.5 text-sm text-gray-500 dark:text-neutral-500">
              ${__2(`Your PWA seems fully setup and your score is excellent.`, slug6)}
            </p>
          </div>
        </div>
      `;
      ...
      <div class="relative mt-5 flex flex-col">
        <h3 class="mb-1.5 text-sm text-gray-500 dark:text-neutral-200">${__2(`Action Items`, slug6)}</h3>
        ...
        ${actionItems.length > 2 ? `
          <button onclick="this.previousElementSibling.classList.add('expanded');this.remove();" class="absolute z-30 bottom-0 self-center py-1.5 px-2.5 inline-flex items-center gap-x-1 text-xs font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 data-[disabled=true]:opacity-50 data-[disabled=true]:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-200 dark:hover:bg-neutral-700 dark:hover:text-blue-500">
            ...
            <span>${__2(`Show All`, slug6)} (${actionItems.length})</span>
          </button>
        ` : ""}
      </div>
    `;
  ...
  function handleAppIconUpload() {
    const settingAppIcon = daftplugAdmin19.find("#settingAppIcon");
  ...
  var optionName2 = daftplugAdmin20.attr("data-option-name");
  ...
  function handleAppShortcutIconUpload() {
    const settingAppShortcuts = daftplugAdmin20.find("#settingAppShortcuts");
  ...
  function populateInitialShortcutIcons() {
    const elements = daftplugAdmin20.find('[data-dp-copy-markup-target^="appShortcut"]');
    elements.each(function(index, el) {
  ...
  var optionName3 = daftplugAdmin21.attr("data-option-name");
  ...
  function handleAppScreenshotsUpload() {
    const settingAppScreenshots = daftplugAdmin21.find("#settingAppScreenshots");
  ...
  var daftplugAdmin22 = jQuery("#daftplugAdmin");
  function initPushImageUpload() {
    handlePushImageUpload();
  }
  function handlePushImageUpload() {
    const sendPushPopup = daftplugAdmin22.find("#send-notification-popup");
  ...
  var daftplugAdmin23 = document.querySelector("#daftplugAdmin");
  var slug7 = daftplugAdmin23.getAttribute("data-slug");
  var { __: __3 } = wp.i18n;
  var SubscriberManager = class {
            ...
            <div class="p-5 min-h-56 flex flex-col justify-center items-center text-center">
              ...
              <div class="max-w-sm mx-auto">
                <p class="mt-2 text-base font-medium text-gray-800 dark:text-neutral-200">
                  ${__3("No Subscribers", slug7)}
                </p>
                <p class="text-sm text-gray-500 dark:text-neutral-500">
                  ${__3("There are no push notification subscribers yet.", slug7)}
                </p>
              </div>
            </div>
          </td>
    ...
    async removeSubscriber(endpoint) {
      if (!confirm(__3("Are you sure you want to remove this subscriber?", slug7))) return;
      try {
  ...
  function initPushSubscribers() {
    const pushSubscribers = daftplugAdmin23.querySelector("#pushNotificationsSubscribers");
  ...
  var daftplugAdmin24 = utils_default("#daftplugAdmin");
  var optionName4 = daftplugAdmin24.attr("data-option-name");
  var slug8 = daftplugAdmin24.attr("data-slug");
  var jsVars4 = window[optionName4 + "_admin_js_vars"];
  var { __: __4 } = wp.i18n;
  function initModalPushNotifications() {
    daftplugAdmin24.find('form[id="send-notification-popup"]').on("submit", doModalPushNotification);
    daftplugAdmin24.find('form[id="send-notification-popup"] #previewPushNotification').on("click", previewPushNotification);
  }
  function doModalPushNotification(e) {
    ...
    const intractableComponents = daftplugAdmin24.find("header, aside, button, footer");
    ...
    fetch(wpApiSettings.root + slug8 + "/doModalPushNotification", {
      ...
      showToast("Fail", error.message || __4("Sending failed. There was an error on server.", slug8), "fail", "top-right", true, false);
    }).finally(() => {
  ...
  function previewPushNotification(e) {
    ...
    if (!("Notification" in window)) {
      showToast("Fail", __4("Notifications are not supported by your browser.", slug8), "fail", "top-right", true, false);
      return;
    }
    if (Notification.permission === "granted") {
      ...
      Notification.requestPermission().then((permission) => {
        if (permission === "granted") {
          ...
          showToast("Fail", __4("You need to accept the notifications permission to preview.", slug8), "fail", "top-right", true, false);
        }
      });
    } else {
      showToast("Fail", __4("Push notifications are blocked by your browser.", slug8), "fail", "top-right", true, false);
    }
  }

  // admin/assets/js/dev/modules/publishToAppStores.js
  var daftplugAdmin25 = jQuery("#daftplugAdmin");
  var slug9 = daftplugAdmin25.attr("data-slug");
  var { __: __5 } = wp.i18n;
  function initPublishToAppStores() {
    const paypalContainers = daftplugAdmin25.find(".paypalButtonsContainer");
    ...
    paypal.Buttons({
      ...
      onApprove: function(data, actions) {
        return actions.order.capture().then(function(orderData) {
          ...
          paypalResponse.css({
            ...
            marginTop: "10px",
            color: "#fff"
          }).html(__5("Thank you for the payment! We are starting to create apps for you and will send it to you soon.", slug9)).fadeIn("fast");
          setTimeout(function() {
      ...
      onError: function(err) {
        ...
        paypalResponse.css({
          ...
          marginTop: "10px",
          color: "#fff"
        }).html(__5("Payment failed! Please try again or contact us on support@daftplug.com", slug9)).fadeIn("fast");
        setTimeout(function() {
          paypalResponse.fadeOut("fast", function() {
            paypalButtons.fadeIn("fast");
          });
  ...
  var daftplugAdmin26 = utils_default("#daftplugAdmin");
  var slug10 = daftplugAdmin26.attr("data-slug");
  function initSupportRequest() {
    daftplugAdmin26.find('form[name="supportForm"]').on("submit", sendSupportRequest);
  }
  async function sendSupportRequest(e) {
    ...
    const submitRequestBtn = form.find('button[type="submit"]');
    const intractableComponents = daftplugAdmin26.find("header, aside, main, footer");
