...
class Dashboard
{
  ...
  public function __construct($config)
  {
    ...
    add_action('init', [$this, 'createPwaUsersTable']);
  ...
  public function fetchPwaUsersData()
  {
    try {
      ...
      if ($tableExists) {
        ...
        $activeUsers = (int) $this->wpdb->get_var($this->wpdb->prepare("SELECT COUNT(*) FROM {$this->tableName} WHERE last_open_date >= %s", $ninetyDaysAgo));
  ...
  public function fetchPwaScoreData()
  {
    $allActionItems = [
      'appStores' => [
        ...
        'title' => esc_html__('Publish your web app to stores', $this->slug),
        'icon' =>
        ...
        'action' => [
          'navigateToPage' => 'publishToAppStores',
        ],
      ],
      'https' => [
        'weight' => 25,
