...
if (!class_exists('ProgressifyWebAppManifest')) {
  class ProgressifyWebAppManifest
  {
    public $name;
    public $description;
    public $slug;
    ...
    public function generateLaunchScreensAndMakableIcons()
    {
      if (isset($this->settings['pwaIcon'])) {
        if (isset($_POST['launchScreen']) && !empty($_POST['launchScreen']) && (isset($_POST['maskableIcon']) && !empty($_POST['maskableIcon']))) {
          ...
          Progressify::putContent($launchScreenName, $decodedLaunchScreen);
          Progressify::putContent($maskableIconName, $decodedMaskableIcon);
