(() => {
  ...
  var getContrastTextColor, isReturningVisitor, urlBase64ToUint8Array, hasUrlParam, addParamToUrl, removeParamFromUrl, setCookie, getCookie;
      ...
      PushNotificationsSubscriptionManager = class {
        ...
        async addSubscription() {
          ...
          try {
            ...
            const subscription = await registration.pushManager.subscribe({
              ...
              applicationServerKey: urlBase64ToUint8Array(config2.jsVars.vapidPublicKey)
            });
            const response = await fetch(`${config2.jsVars.restUrl}${config2.jsVars.slug}/addSubscription`, {
              method: "POST",
              credentials: "include",
              headers: {
        ...
        async removeSubscription() {
          ...
          try {
            ...
            const response = await fetch(`${config2.jsVars.restUrl}${config2.jsVars.slug}/removeSubscription`, {
              method: "DELETE",
              credentials: "include",
              headers: {
                "Content-Type": "application/json"
  ...
  async function sendPwaUsageEventToServer(retries = 3) {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        const response = await fetch(`${config2.jsVars.restUrl}${config2.jsVars.slug}/upsertPwaUser`, {
          method: "PUT",
          credentials: "include",
          headers: {
            "Content-Type": "application/json"
  ...
  var init_pageLoader = __esm({
    "frontend/assets/js/dev/modules/pageLoader.js"() {
      ...
      PwaPageLoader = class extends HTMLElement {
        constructor() {
          ...
          this.type = config2.jsVars.settings.uiComponents.pageLoader.type;
    ...
    `;
        ...
        renderDefaultPageLoader() {
          const backgroundColor = config2.jsVars.settings.webAppManifest?.appearance?.backgroundColor ?? "#ffffff";
          const appIcon = config2.jsVars.iconUrl ?? "";
          this.injectStyles(`
      .pageLoader.-default {
  ...
  // frontend/assets/js/dev/components/i18n.js
  function __2(text, textDomain) {
  ...
  var getTranslations, translations;
  var init_i18n = __esm({
    "frontend/assets/js/dev/components/i18n.js"() {
      init_frontend();
      getTranslations = () => {
        try {
          return typeof config2.jsVars.translations === "string" ? JSON.parse(config2.jsVars.translations) : config2.jsVars.translations || {};
        } catch (e) {
  ...
  var init_installPrompt = __esm({
    "frontend/assets/js/dev/components/installPrompt.js"() {
      ...
      PwaInstallPrompt = class extends HTMLElement {
        ...
        renderAppIcon() {
          if (!config2.jsVars.iconUrl) return "";
          this.injectStyles(`
    ...
    `);
          const appName = config2.jsVars.settings.webAppManifest.appIdentity.appName;
          return `
      <img 
        class="install-prompt-body-appinfo_icon" 
        src="${config2.jsVars.iconUrl}" 
        alt="${appName}"
    ...
    `);
          const description = config2.jsVars.settings.webAppManifest.appIdentity.description;
          return description ? `
    ...
    `);
          const startPage = config2.jsVars.settings.webAppManifest.displaySettings?.startPage;
          return startPage ? `
      <button type="button" class="install-prompt-body-instructions_step_copy" data-clipboard-content="${addParamToUrl("performInstallation", "true", startPage)}">
        ...
        <span class="install-prompt-body-instructions_step_copy_tooltip" role="tooltip">
          ${__2("Copied", config2.jsVars.slug)}
        </span>
      </button>
    ` : "";
        }
    ...
    `);
      ...
      <div class="install-prompt-loading">
        ...
        <div class="install-prompt-loading-text">${__2("Checking installation capabilities...", config2.jsVars.slug)}</div>
      </div>
    `;
        }
        renderNativeInstallButton() {
          const themeColor = config2.jsVars.settings.webAppManifest?.appearance?.themeColor ?? "#000000";
    ...
    `);
          const { device, os, browser } = config2.jsVars.userData;
          let btnIconSvg = "";
          let btnIconText = __2("Click to Install", config2.jsVars.slug);
          if (device.isSmartphone || device.isTablet) {
            btnIconSvg = svg.installMobile;
            btnIconText = __2("Tap to Install", config2.jsVars.slug);
    ...
    `);
          ...
          if (device.isSmartphone || device.isTablet) {
            if (os.isAndroid) {
              if (browser.isChrome) {
                renderStep(__2("Tap the Menu Icon", config2.jsVars.slug), __2("Tap the menu icon (three dots), located at the top-right of your screen.", config2.jsVars.slug), svg.vertThreeDots);
                renderStep(__2('Select "Add to Home Screen"', config2.jsVars.slug), __2('Tap the "Add to Home Screen" option from the menu.', config2.jsVars.slug), svg.addToHomeScreen);
                renderStep(__2('Confirm by Tapping "Add"', config2.jsVars.slug), __2('Tap the "Add" text on the browser installation dialog.', config2.jsVars.slug), svg.tapFinger);
              } else if (browser.isFirefox) {
                renderStep(__2("Tap the Menu Icon", config2.jsVars.slug), __2("Tap the menu icon (three dots), located at the top-right of your screen.", config2.jsVars.slug), svg.vertThreeDots);
                renderStep(__2('Select "Install"', config2.jsVars.slug), __2('Tap the "Install" option from the menu.', config2.jsVars.slug), svg.addToHomeScreen);
                renderStep(__2('Confirm by Tapping "Add to Home Screen"', config2.jsVars.slug), __2('Tap the "Add to Home Screen" button on the browser installation dialog.', config2.jsVars.slug), svg.tapFinger);
              } else if (browser.isOpera) {
                renderStep(__2("Tap the Menu Icon", config2.jsVars.slug), __2("Tap the menu icon (three dots), located at the top-right of your screen.", config2.jsVars.slug), svg.vertThreeDots);
                renderStep(__2('Select "Home Screen"', config2.jsVars.slug), __2('Tap the "Install" option from the menu.', config2.jsVars.slug), svg.installMobile);
                renderStep(__2('Confirm by Tapping "Add"', config2.jsVars.slug), __2('Tap the "Add" text on the browser installation dialog.', config2.jsVars.slug), svg.tapFinger);
              } else if (browser.isEdge) {
                renderStep(__2("Tap the Menu Icon", config2.jsVars.slug), __2("Tap the menu icon (three horizontal lines) located at the bottom-right of your screen.", config2.jsVars.slug), svg.hamburgerMenu);
                renderStep(__2('Select "Add to Phone"', config2.jsVars.slug), __2('Tap the "Add to Phone" option from the menu', config2.jsVars.slug), svg.installMobile);
                renderStep(__2('Confirm by Tapping "Install"', config2.jsVars.slug), __2('Tap the "Install" text on the browser installation dialog.', config2.jsVars.slug), svg.tapFinger);
              } else {
                renderStep(__2("Copy the Page URL", config2.jsVars.slug), __2("Click the button below to copy the page's URL.", config2.jsVars.slug), svg.copy, this.renderCopyInstallUrlButton());
                renderStep(__2("Open the Google Chrome Browser", config2.jsVars.slug), __2("Launch the Google Chrome web browser from your home screen.", config2.jsVars.slug), svg.googleChrome);
                renderStep(__2("Paste and Open the URL", config2.jsVars.slug), __2("Paste the copied URL into Google Chrome's address bar and open the page.", config2.jsVars.slug), svg.pasteGo);
              }
            } else if (os.isIos) {
              if (browser.isSafari) {
                renderStep(__2("Tap the Share Icon", config2.jsVars.slug), __2("Tap the share icon located at the center-bottom of your screen.", config2.jsVars.slug), svg.iosShare);
                renderStep(__2('Select "Add to Home Screen"', config2.jsVars.slug), __2('Tap the "Add to Home Screen" option, represented by the plus [+] icon.', config2.jsVars.slug), svg.plusInBox);
                renderStep(__2('Confirm by Tapping "Add"', config2.jsVars.slug), __2('Tap the "Add" button at the top-right corner of your screen.', config2.jsVars.slug), svg.tapFinger);
              } else if (browser.isChrome) {
                renderStep(__2("Tap the Share Icon", config2.jsVars.slug), __2("Tap the share icon located at the top-right of your browser address bar.", config2.jsVars.slug), svg.iosShare);
                renderStep(__2('Select "Add to Home Screen"', config2.jsVars.slug), __2('Tap the "Add to Home Screen" option, represented by the plus [+] icon.', config2.jsVars.slug), svg.plusInBox);
                renderStep(__2('Confirm by Tapping "Add"', config2.jsVars.slug), __2('Tap the "Add" button at the top-right corner of your screen.', config2.jsVars.slug), svg.tapFinger);
              } else if (browser.isFirefox) {
                renderStep(__2("Tap the Menu Icon", config2.jsVars.slug), __2("Tap the menu icon (three horizontal lines) located at the bottom-right of your screen.", config2.jsVars.slug), svg.hamburgerMenu);
                renderStep(__2("Tap the Share Icon", config2.jsVars.slug), __2("Tap the share icon in the dialog overlay that the browser opens.", config2.jsVars.slug), svg.iosShare);
                renderStep(__2('Select "Add to Home Screen"', config2.jsVars.slug), __2('Tap the "Add to Home Screen" option, represented by the plus [+] icon.', config2.jsVars.slug), svg.plusInBox);
                renderStep(__2('Confirm by Tapping "Add"', config2.jsVars.slug), __2('Tap the "Add" button at the top-right corner of your screen.', config2.jsVars.slug), svg.tapFinger);
              } else if (browser.isOpera) {
                renderStep(__2("Tap the Menu Icon", config2.jsVars.slug), __2("Tap the menu icon (three dots), located at the bottom-right of your screen.", config2.jsVars.slug), svg.vertThreeDots);
                renderStep(__2("Tap the Share Icon", config2.jsVars.slug), __2("Tap the share icon located at the top of the dialog overlay that browser opens.", config2.jsVars.slug), svg.upload);
                renderStep(__2('Select "Add to Home Screen"', config2.jsVars.slug), __2('Tap the "Add to Home Screen" option, represented by the plus [+] icon.', config2.jsVars.slug), svg.plusInBox);
                renderStep(__2('Confirm by Tapping "Add"', config2.jsVars.slug), __2('Tap the "Add" button at the top-right corner of your screen.', config2.jsVars.slug), svg.tapFinger);
              } else if (browser.isEdge) {
                renderStep(__2("Tap the Menu Icon", config2.jsVars.slug), __2("Tap the menu icon (three horizontal lines) located at the bottom-right of your screen.", config2.jsVars.slug), svg.hamburgerMenu);
                renderStep(__2("Tap the Share Icon", config2.jsVars.slug), __2("Tap the share icon in the dialog overlay that the browser opens.", config2.jsVars.slug), svg.upload);
                renderStep(__2('Select "Add to Home Screen"', config2.jsVars.slug), __2('Tap the "Add to Home Screen" option, represented by the plus [+] icon.', config2.jsVars.slug), svg.plusInBox);
                renderStep(__2('Confirm by Tapping "Add"', config2.jsVars.slug), __2('Tap the "Add" button at the top-right corner of your screen.', config2.jsVars.slug), svg.tapFinger);
              } else if (browser.isDuckduckgo) {
                renderStep(__2("Tap the Share Icon", config2.jsVars.slug), __2("Tap the share icon located at the right side of the browser's address bar.", config2.jsVars.slug), svg.upload);
                renderStep(__2('Select "Add to Home Screen"', config2.jsVars.slug), __2('Tap the "Add to Home Screen" option, represented by the plus [+] icon.', config2.jsVars.slug), svg.plusInBox);
                renderStep(__2('Confirm by Tapping "Add"', config2.jsVars.slug), __2('Tap the "Add" button at the top-right corner of your screen.', config2.jsVars.slug), svg.tapFinger);
              } else {
                renderStep(__2("Copy the Page URL", config2.jsVars.slug), __2("Click the button below to copy the page's URL.", config2.jsVars.slug), svg.copy, this.renderCopyInstallUrlButton());
                renderStep(__2("Open the Safari Browser", config2.jsVars.slug), __2("Launch the Safari web browser from your home screen.", config2.jsVars.slug), svg.safari);
                renderStep(__2("Paste and Open the URL", config2.jsVars.slug), __2("Paste the copied URL into Safari's address bar and open the page.", config2.jsVars.slug), svg.pasteGo);
              }
            } else {
              renderStep(__2("Installation Not Supported", config2.jsVars.slug), __2("Your operating system does not support web app installation. Please try accessing the website on an Android or iOS mobile device.", config2.jsVars.slug), svg.noInstallSupport);
            }
          } else if (device.isDesktop) {
            if (browser.isChrome) {
              renderStep(__2("Tap the Menu Icon", config2.jsVars.slug), __2("Tap the menu icon (three dots) in the top-right corner of your browser window.", config2.jsVars.slug), svg.vertThreeDots);
              renderStep(__2('Expand "Cast, save and share"', config2.jsVars.slug), __2('Hover over the "Cast, Save, and Share" menu item to expand its options.', config2.jsVars.slug), svg.fileSave);
              renderStep(__2('Select "Install page as app..."', config2.jsVars.slug), __2('Click on "Install page as app..." from the menu.', config2.jsVars.slug), svg.installDesktop);
              renderStep(__2('Confirm by Clicking "Install"', config2.jsVars.slug), __2(`Click the "Install" button in the browser's installation dialog.`, config2.jsVars.slug), svg.mouseClick);
            } else if (browser.isEdge) {
              renderStep(__2("Tap the Menu Icon", config2.jsVars.slug), __2("Tap the menu icon (three dots) in the top-right corner of your browser window.", config2.jsVars.slug), svg.horizThreeDots);
              renderStep(__2('Expand "Apps"', config2.jsVars.slug), __2('Hover over the "Apps" menu item to expand its options.', config2.jsVars.slug), svg.appsGrid);
              renderStep(__2('Select "Install this site as app"', config2.jsVars.slug), __2('Click on "Install this site as app" from the menu.', config2.jsVars.slug), svg.directionDown);
              renderStep(__2('Confirm by Clicking "Install"', config2.jsVars.slug), __2(`Click the "Install" button in the browser's installation dialog.`, config2.jsVars.slug), svg.mouseClick);
            } else {
              renderStep(__2("Copy the Page URL", config2.jsVars.slug), __2("Click the button below to copy the page's URL.", config2.jsVars.slug), svg.copy, this.renderCopyInstallUrlButton());
              renderStep(__2("Open the Google Chrome Browser", config2.jsVars.slug), __2("Launch the Google Chrome web browser from your start menu.", config2.jsVars.slug), svg.googleChrome);
              renderStep(__2("Paste and Open the URL", config2.jsVars.slug), __2("Paste the copied URL into Google Chrome's address bar and open the page.", config2.jsVars.slug), svg.pasteGo);
            }
          } else {
            renderStep(__2("Installation Not Supported", config2.jsVars.slug), __2("Your device does not support web app installation. Please try accessing the website on a mobile or desktop device.", config2.jsVars.slug), svg.noInstallSupport);
          }
    ...
    `);
          ...
          const promptTitle = config2.jsVars.settings.installation?.prompts?.text ?? __2("Install Web App", config2.jsVars.slug);
          const combinedStyles = Array.from(this.styles).join("\n");
          this.shadowRoot.innerHTML = `
        ...
        <div class="install-prompt-container">
          ...
          <div class="install-prompt-footer">
            <button type="button" class="install-prompt-footer_close">
              ${__2("Close Dialog", config2.jsVars.slug)}
            </button>
          </div>
        </div>
      </div>
    ...
    "frontend/assets/js/dev/modules/installButton.js"() {
      ...
      PwaInstallButton = class extends HTMLElement {
        ...
        render() {
          const themeColor = config2.jsVars.settings.webAppManifest?.appearance?.themeColor ?? "#000000";
          const textColor = getContrastTextColor(themeColor);
          const buttonText = __("Add to Home Screen", config2.jsVars.slug);
          this.injectStyles(`
      :host(:active),
      :host(:focus) {
        outline: transparent;
    ...
    "frontend/assets/js/dev/modules/installOverlayHeaderBanner.js"() {
      ...
      PwaInstallOverlayHeaderBanner = class extends HTMLElement {
        ...
        render() {
          ...
          const themeColor = config2.jsVars.settings.webAppManifest?.appearance?.themeColor ?? "#000000";
          const textColor = getContrastTextColor(themeColor);
          const bannerTitle = config2.jsVars.settings.installation?.prompts?.types?.headerBanner?.title ?? __2("Install Web App", config2.jsVars.slug);
          const bannerMessage = config2.jsVars.settings.installation?.prompts?.types?.headerBanner?.message ?? __2("Get our web app. It won't take up space on your device.", config2.jsVars.slug);
          const bannerButtonText = config2.jsVars.settings.installation?.prompts?.types?.headerBanner?.buttonText ?? __2("Install Now", config2.jsVars.slug);
          const appIconHtml = config2.jsVars.iconUrl ? `<img class="header-banner-overlay-appinfo_icon" src="${config2.jsVars.iconUrl}" alt="${appName}" onerror="this.style.display='none'"></img>` : "";
          this.injectStyles(`
    ...
    "frontend/assets/js/dev/modules/installOverlaySnackbar.js"() {
      ...
      PwaInstallOverlaySnackbar = class extends HTMLElement {
        ...
        render() {
          const themeColor = config2.jsVars.settings.webAppManifest?.appearance?.themeColor ?? "#000000";
          const textColor = getContrastTextColor(themeColor);
          const snackbarTitle = config2.jsVars.settings.installation?.prompts?.types?.snackbar?.title ?? __2("Install Web App", config2.jsVars.slug);
          const snackbarMessage = config2.jsVars.settings.installation?.prompts?.types?.snackbar?.message ?? __2("Installing uses no storage and offers a quick way back to our web app.", config2.jsVars.slug);
          const snackbarButtonText = config2.jsVars.settings.installation?.prompts?.types?.snackbar?.buttonText ?? __2("Install Now", config2.jsVars.slug);
          this.injectStyles(`
      .snackbar-overlay {
        display: flex;
        align-items: center;
    ...
    "frontend/assets/js/dev/modules/installOverlayBlogPopup.js"() {
      ...
      PwaInstallOverlayBlogPopup = class extends HTMLElement {
        ...
        render() {
          ...
          const themeColor = config2.jsVars.settings.webAppManifest?.appearance?.themeColor ?? "#000000";
          const textColor = getContrastTextColor(themeColor);
          const popupTitle = config2.jsVars.settings.installation?.prompts?.types?.blogPopup?.title ?? __2("Read this article in our web app", config2.jsVars.slug);
          const popupButtonText = config2.jsVars.settings.installation?.prompts?.types?.blogPopup?.buttonText ?? __2("Open", config2.jsVars.slug);
          const popupButtonTextContinue = config2.jsVars.settings.installation?.prompts?.types?.blogPopup?.buttonTextContinue ?? __2("Continue", config2.jsVars.slug);
          const appIconHtml = config2.jsVars.iconUrl ? `<img class="blog-popup-overlay-appinfo_icon" src="${config2.jsVars.iconUrl}" alt="${appName}" onerror="this.style.display='none'"></img>` : "";
          let browserTitle;
          let browserIcon;
          switch (true) {
            case browser.isChrome:
              browserTitle = "Chrome";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/chrome.png";
              break;
            case browser.isSafari:
              browserTitle = "Safari";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/safari.png";
              break;
            case browser.isFirefox:
              browserTitle = "Firefox";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/firefox.png";
              break;
            case browser.isOpera:
              browserTitle = "Opera";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/opera.png";
              break;
            case browser.isEdge:
              browserTitle = "Edge";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/edge.png";
              break;
            case browser.isSamsung:
              browserTitle = "Samsung Browser";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/samsung.png";
              break;
            case browser.isYandex:
              browserTitle = "Yandex Browser";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/yandex.png";
              break;
            case browser.isDuckduckgo:
              browserTitle = "DuckDuckGo";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/duckduckgo.png";
              break;
            case browser.isBrave:
              browserTitle = "Brave";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/brave.png";
              break;
            case browser.isQq:
              browserTitle = "QQ Browser";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/qq.png";
              break;
            case browser.isUc:
              browserTitle = "UC Browser";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/browsers/uc.png";
              break;
            default:
              browserTitle = "Browser";
              browserIcon = config2.jsVars.pluginsData.dirUrl + "/frontend/assets/media/icons/unknown.png";
              break;
          }
          this.injectStyles(`
      .blog-popup-overlay {
    ...
    "frontend/assets/js/dev/modules/installOverlayNavigationMenu.js"() {
      ...
      PwaInstallOverlayNavigationMenu = class extends HTMLElement {
        ...
        render() {
          ...
          const themeColor = config2.jsVars.settings.webAppManifest?.appearance?.themeColor ?? "#000000";
          const textColor = getContrastTextColor(themeColor);
          const navigationMenuTitle = config2.jsVars.settings.installation?.prompts?.types?.navigationMenu?.title ?? __2("Install Our Web App", config2.jsVars.slug);
          const navigationMenuMessage = config2.jsVars.settings.installation?.prompts?.types?.navigationMenu?.message ?? __2("Find what you need faster by installing our web app!", config2.jsVars.slug);
          const navigationMenuButtonText = config2.jsVars.settings.installation?.prompts?.types?.navigationMenu?.buttonText ?? __2("Install Now", config2.jsVars.slug);
          const appIconHtml = config2.jsVars.iconUrl ? `<img class="navigation-menu-overlay-appinfo_icon" src="${config2.jsVars.iconUrl}" alt="${appName}" onerror="this.style.display='none'"/>` : "";
          this.injectStyles(`
    ...
    "frontend/assets/js/dev/modules/installOverlayInFeed.js"() {
      ...
      PwaInstallOverlayInFeed = class extends HTMLElement {
        ...
        render() {
          ...
          const themeColor = config2.jsVars.settings.webAppManifest?.appearance?.themeColor ?? "#000000";
          const textColor = getContrastTextColor(themeColor);
          const inFeedTitle = config2.jsVars.settings.installation?.prompts?.types?.inFeed?.title ?? __2("Install on Home Screen", config2.jsVars.slug);
          const inFeedMessage = config2.jsVars.settings.installation?.prompts?.types?.inFeed?.message ?? __2("Keep reading, even when you're on the train!", config2.jsVars.slug);
          const inFeedButtonText = config2.jsVars.settings.installation?.prompts?.types?.inFeed?.buttonText ?? __2("Install Now", config2.jsVars.slug);
          const appIconHtml = config2.jsVars.iconUrl ? `<img class="in-feed-overlay-appinfo_icon" src="${config2.jsVars.iconUrl}" alt="${appName}" onerror="this.style.display='none'"/>` : "";
          this.injectStyles(`
      .in-feed-overlay {
        position: relative;
        border-radius: 0.5rem;
    ...
    "frontend/assets/js/dev/modules/installOverlayWoocommerceCheckout.js"() {
      ...
      PwaInstallOverlayWoocommerceCheckout = class extends HTMLElement {
        ...
        render() {
          ...
          const themeColor = config2.jsVars.settings.webAppManifest?.appearance?.themeColor ?? "#000000";
          const textColor = getContrastTextColor(themeColor);
          const woocommerceCheckoutTitle = config2.jsVars.settings.installation?.prompts?.types?.woocommerceCheckout?.title ?? __2("Add to Home Screen", config2.jsVars.slug);
          const woocommerceCheckoutMessage = config2.jsVars.settings.installation?.prompts?.types?.woocommerceCheckout?.message ?? __2("Keep track of your orders. Our web app is fast, small and works offline.", config2.jsVars.slug);
          const woocommerceCheckoutButtonText = config2.jsVars.settings.installation?.prompts?.types?.woocommerceCheckout?.buttonText ?? __2("Install Now", config2.jsVars.slug);
          const appIconHtml = config2.jsVars.iconUrl ? `<img class="woocommerce-checkout-overlay-appinfo_icon" src="${config2.jsVars.iconUrl}" alt="${appName}" onerror="this.style.display='none'"/>` : "";
          this.injectStyles(`
      ...
      PwaOfflineNotification = class extends HTMLElement {
        ...
        static showReconnecting() {
          ...
          requestAnimationFrame(() => {
            ...
            text.textContent = __2("Connection lost. Attempting to reconnect...", config2.jsVars.slug);
            setTimeout(() => {
        ...
        static showReconnected() {
          ...
          text.textContent = __2("Successfully reconnected to the internet!", config2.jsVars.slug);
    ...
    `);
      ...
      <div class="offline-notification" role="alert" tabindex="-1">
        ...
        <span class="offline-notification_text">
          ${__2("Connection lost. Attempting to reconnect...", config2.jsVars.slug)}
        </span>
      </div>
    `;
        }
        ...
        <div class="offline-form-handler-container">
          <div class="offline-form-handler-header">
            <div class="offline-form-handler-header-texts">
              ...
              <div class="offline-form-handler-header-texts_title">${__2("No Internet Connection", config2.jsVars.slug)}</div>
            </div>
          </div>
          <div class="offline-form-handler-body">
            <div class="offline-form-handler-body_message">${__2("You\u2019re currently offline. Your form submission data will be saved and be automatically processed in the background when you\u2019re back online within 24 hours. Would you like to proceed?", config2.jsVars.slug)}</div>
          </div>
  ...
  var svgIcons, PwaNavigationTabBar;
  var init_navigationTabBar = __esm({
    "frontend/assets/js/dev/modules/navigationTabBar.js"() {
      ...
      PwaNavigationTabBar = class extends HTMLElement {
        ...
        renderTabBarItems() {
          const navigationItems = config2.jsVars.settings.uiComponents.navigationTabBar.navigationItems || [];
  ...
  var init_darkMode = __esm({
    "frontend/assets/js/dev/modules/darkMode.js"() {
      ...
      PwaDarkMode = class extends HTMLElement {
        constructor() {
          ...
          this.switchType = config2.jsVars.settings.uiComponents.darkMode.type;
          this.icons = {
    ...
    `;
        ...
        static show() {
          ...
          const switchType = config2.jsVars.settings.uiComponents.darkMode.type;
          if (!darkModeSwitch) {
        ...
        async checkBatteryLevel() {
          if (config2.jsVars.settings.uiComponents.darkMode.batteryLow === "on" && "getBattery" in navigator) {
            try {
        ...
        checkSystemPreference() {
          if (config2.jsVars.settings.uiComponents.darkMode.osAware === "on") {
    ...
    `);
          return `
      <div class="dark-mode-menu ${this.darkMode ? "-dark" : ""}" aria-label="${__2("Toggle dark mode")}">
    ...
    `);
          return `
      <div class="dark-mode-floating" aria-label="${__2("Toggle dark mode")}">
      ...
      PwaPullDownRefresh = class extends HTMLElement {
        ...
        handleTouchMove(e) {
          ...
          if (pullDistance >= 0) {
            ...
            if (pullDistance > this.threshold) {
              statusText.textContent = __2("Release to refresh", config2.jsVars.slug);
            } else {
              statusText.textContent = __2("Pull down to refresh", config2.jsVars.slug);
            }
          }
        }
        async handleTouchEnd() {
          ...
          if (pullDistance > this.threshold) {
            ...
            statusText.textContent = __2("Refreshing...", config2.jsVars.slug);
      ...
      <div class="pull-down-refresh" role="alert">
        <div class="pull-down-refresh_content">
          ...
          <span class="pull-down-refresh_text">
            ${__2("Pull down to refresh", config2.jsVars.slug)}
          </span>
        </div>
      </div>
    `;
    ...
    `);
      ...
      <div class="shake-notification" role="alert">
        ...
        <span class="shake-notification_text">
          ${__2("Shake detected! Refreshing the page...", config2.jsVars.slug)}
        </span>
      </div>
    `;
        }
  ...
  var init_toastMessages = __esm({
    ...
    `);
          let message;
          switch (true) {
            case config2.jsVars.pageData.type.isHome:
              message = __2("Homepage Opened", config2.jsVars.slug);
              break;
            case config2.jsVars.pageData.type.is404:
              message = __2("Page Not Found", config2.jsVars.slug);
              break;
            case config2.jsVars.pageData.type.isSearch:
              message = __2("Search Results", config2.jsVars.slug);
              break;
            case config2.jsVars.pageData.type.isBlogPost:
              message = __2("Article Opened", config2.jsVars.slug);
              break;
            case config2.jsVars.pageData.type.isWooShop:
              message = __2("Shop Opened", config2.jsVars.slug);
              break;
            case config2.jsVars.pageData.type.isWooProduct:
              message = __2("Product Opened", config2.jsVars.slug);
              break;
            default:
              message = __2("Page Opened", config2.jsVars.slug);
              break;
          }
          const combinedStyles = Array.from(this.styles).join("\n");
          this.shadowRoot.innerHTML = `
  ...
  async function initSmoothPageTransitions() {
    ...
    switch (config2.jsVars.pageData.builder) {
      case "elementor":
    ...
    if (config2.jsVars.settings.appCapabilities.smoothPageTransitions.progressBar === "on") {
      ...
      document.head.insertAdjacentHTML("beforeend", "<style>.swup-progress-bar { background-color: " + config2.jsVars.settings.webAppManifest.appearance.themeColor + " !important; }</style>");
    }
    if (config2.jsVars.settings.appCapabilities.smoothPageTransitions.transition === "slide") {
    ...
    if (config2.jsVars.settings.appCapabilities.smoothPageTransitions.compatibilityMode === "on") {
        ...
        if (state === "granted") {
          try {
            ...
            await idleDetector.start({
              threshold: config2.jsVars.settings.appCapabilities.idleDetection.threshold * 6e4,
      ...
      <div class="idle-notification" role="alert" tabindex="-1">
        <div class="idle-notification_texts">
          ...
          <span class="idle-notification_text">
            ${__2("You've been idle for a while.", config2.jsVars.slug)}
          </span>
        </div>
    ...
    "frontend/assets/js/dev/modules/pushNotificationsPrompt.js"() {
      ...
      PwaPushNotificationsPrompt = class _PwaPushNotificationsPrompt extends HTMLElement {
        ...
        render() {
          ...
          const backgroundColor = getContrastTextColor(themeColor);
          const textColor = getContrastTextColor(backgroundColor);
          const promptMessage = config2.jsVars.settings.pushNotifications?.prompt?.message ?? __2("We would like to show you notifications for the latest news and updates.", config2.jsVars.slug);
          const appName = config2.jsVars.settings.webAppManifest.appIdentity.appName ?? "";
          const appIconHtml = config2.jsVars.iconUrl ? `<img class="push-notifications-prompt-media_icon" src="${config2.jsVars.iconUrl}" alt="${appName}" onerror="this.style.display='none'"/>` : "";
          this.injectStyles(`
      ...
      <div class="push-notifications-prompt">
        <div class="push-notifications-prompt-media">
          ...
          <div class="push-notifications-prompt-media_texts">
            <div class="push-notifications-prompt-media_title">${__2("Push Notifications", config2.jsVars.slug)}</div>
        ...
        <div class="push-notifications-prompt-buttons">
          <button type="button" class="push-notifications-prompt-button_dismiss">
            ${__2("Dismiss", config2.jsVars.slug)}
          </button>
          <button type="button" class="push-notifications-prompt-button_allow">
            ${__2("Allow Notifications", config2.jsVars.slug)}
          </button>
        </div>
      </div>
    `;
  ...
  var init_pushNotificationsButton = __esm({
    "frontend/assets/js/dev/modules/pushNotificationsButton.js"() {
      ...
      PwaPushNotificationsButton = class extends HTMLElement {
        constructor() {
          ...
          this.attachShadow({ mode: "open" });
          this.styles = /* @__PURE__ */ new Set();
          this.position = config2.jsVars.settings.pushNotifications.button.position;
          this.behavior = config2.jsVars.settings.pushNotifications.button.behavior;
          this.icons = {
    ...
    `);
          ...
          this.shadowRoot.innerHTML = `
      <style>${combinedStyles}</style>
      <div class="push-notifications-button" aria-label="${__2("Enable / Disable Push Notifications")}">
  ...
  var config2, settings, userData, pluginsData, pageData;
  var init_frontend = __esm({
    "frontend/assets/js/dev/frontend.js"() {
      ...
      ({ settings, userData, pluginsData, pageData } = config2.jsVars);
